...
static CompData *MakeFam(NodeData *Nodes)
	{
	CompData *Fam = new CompData;
	for (const NodeData *Node = Nodes; Node; Node = Node->Next)
		{
		CompMemberData CompMember;
		CompMember.PileIndex = Node->Index;
		switch (Node->Rev)
			{
...
int GetConnComps(EdgeList &Edges, CompList &Fams, int MinComponentSize)
	...
	// By convention, the first member defines reversal or lack thereof.
		...
		if (ClassSize >= MinComponentSize)
			{
			CompData *Fam = MakeFam(CurrentList);
			Fams.push_back(Fam);
			++FamCount;
			}
